...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment & { deployments: DeploymentsExtension }): Promise<void> => {
    let _deployer;
    ...
    if (allowance < sum) {
        // Approve the factory for the sum of all funding
        const populated = await token.getFunction("approve").populateTransaction(factoryDeployment.address, sum - allowance)
    ...
    for (const { beneficiary, cliffDuration, duration, startTime, funding } of toDeploy) {
        const populated = await factory.getFunction("deployTimelock").populateTransaction(
